#!/usr/bin/env python3
"""
animal2vec (data2vec_multi) embedding extractor -- modern torch 2.x env.
Run from the repo root with: uv run python animal2vec_validation/a2v_extract.py ...
...
NOTE: --lab-dir below defaults to a path from the original research environment
(/mnt/c/Users/Iaroslav/CETACEANS/new_training_data) -- override it for your own checkout.
"""
import os, sys, argparse, glob, collections, time, re
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # repo root, for `import animal2vec`
...
import animal2vec.nn as nn  # noqa: F401  (registers data2vec_multi + audio_ccas task)
...
try:
    import animal2vec.nn.modalities.base as _nb
    _swallow_kwargs(_nb, "compute_mask_indices")
except Exception as _e:
    print("compute_mask_indices patch skipped:", _e)

SR = 8000  # default; auto-overridden per-checkpoint by _detect_set_sr() in load_model()
DEV = "cuda" if torch.cuda.is_available() else "cpu"
t0 = time.time(); log = lambda m: print(f"[{time.time()-t0:6.1f}s] {m}", flush=True)


def _detect_set_sr(cfg):
    """Set the global SR from the checkpoint's sample_rate so 16 kHz models get 16 kHz input (the
    sinc kernels are rate-specific -- feeding 8 kHz audio to a 16 kHz model misreads every
    frequency by 2x). cfg = ck['cfg']. Mirrors the same fix already applied in
    docs/marine_mammal_ssl/validation/validate.py (hotfix/validate-16khz)."""
    global SR
    rate = None
...
def sanitize_and_save(src, dst):
    ...
    for heavy in ['last_optimizer_state']:
        if heavy in ck: del ck[heavy]
    log("saving slim checkpoint (skip_ema, _ema + optimizer dropped, ~1.3GB) ...")
    torch.save(ck, dst)
    return dst
