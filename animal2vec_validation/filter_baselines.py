...
Run AFTER a2v_filter.py (separate process => one model on GPU).
Run: uv run python animal2vec_validation/filter_baselines.py"""
import os, time, json
...
M = json.load(open(os.path.join(HERE, "filter_manifest.json")))
...
json.dump(res, open(os.path.join(HERE, "filter_baselines.json"), "w"), indent=2)
