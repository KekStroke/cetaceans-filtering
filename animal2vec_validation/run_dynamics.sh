...
#     "blue:30000:/path/to/blue_30k.pt" \
#     "orange:30000:/path/to/orange_30k.pt"
...
# on load, but pre-slimming keeps RAM low).
set -e
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$HERE/.."
export TF_CPP_MIN_LOG_LEVEL=3
PY="${A2V_PYTHON:-python3}"
VD="$HERE"
for spec in "$@"; do
  ...
  $PY - "$TAG" "$RUN" "$STEP" "$VD" <<'PY'
import json, os, sys
p = os.path.join(sys.argv[4], "dynamics_registry.json")
