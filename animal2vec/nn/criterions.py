...
from animal2vec.nn import confusion, sigmoid_focal_loss, ConcatTensorMeter
...
class ExpandedModelCriterion(ModelCriterion):
    ...
    def __init__(self, task, loss_weights=None, log_keys=None, can_sum=True):
        self.can_sum_original = can_sum
        init_params = inspect.signature(ModelCriterion.__init__).parameters
        kwargs = {}
        if "loss_weights" in init_params:
            kwargs["loss_weights"] = loss_weights
        if "log_keys" in init_params:
            kwargs["log_keys"] = log_keys
        if "can_sum" in init_params:
            kwargs["can_sum"] = can_sum
