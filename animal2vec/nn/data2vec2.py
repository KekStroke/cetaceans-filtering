...
class Data2VecMultiConfig(FairseqDataclass):
    ...
    mixing_gain_sample_rate: Optional[int] = None
...
class Data2VecMultiModel(BaseFairseqModel, FusedSegmentationMixin):
    ...
    def make_ema_teacher(self, ema_decay):
        ...
        ema_params = inspect.signature(EMAModuleConfig).parameters
        ema_config = EMAModuleConfig(
            **{key: value for key, value in ema_kwargs.items() if key in ema_params}
        )

        model_copy = self.make_target_model()
        model_copy.task = None

        ema_module_params = inspect.signature(EMAModule).parameters
        ema_module_kwargs = {"copy_model": False}
        ema = EMAModule(
            model_copy,
            ema_config,
            **{
                key: value
                for key, value in ema_module_kwargs.items()
                if key in ema_module_params
            },
        )
        if not hasattr(ema, "logs"):
            ema.logs = {}
        return ema
    ...
    def set_num_updates(self, num_updates):
        ...
        if self.ema is not None and (
            ...
            if self.cfg.ema_decay != self.cfg.ema_end_decay:
                ...
                set_decay_params = inspect.signature(self.ema.set_decay).parameters
                if "weight_decay" in set_decay_params:
    ...
    def compute_gain_torch(self, sound, fs=8_000, wl=0.1, min_db=-80.0, mode="A_weighting"):
        ...
        if mode == "A_weighting":
            ...
            weight_key = (fs, n_fft)
            if weight_key not in self.a_weight:
                ...
                self.a_weight[weight_key] = torch.from_numpy(
        ...
        if mode == "RMSE":
            ...
            a_weighted_spec = power_spec * self.a_weight[(fs, n_fft)]
    ...
    def forward(
            ...
            with torch.no_grad():
                ...
                if self.cfg.gain_mode == "none":
                    ...
                    if self.cfg.gain_mode == "naive_rms":
                        ...
                        gain_sample_rate = self.cfg.mixing_gain_sample_rate or self.cfg.sample_rate
        ...
        with torch.no_grad():
            ...
            if self.ema is not None:
                for k, v in getattr(self.ema, "logs", {}).items():
                    result[k] = v
