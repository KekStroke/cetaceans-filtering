...
from animal2vec.nn import SincConv
...
class ExplicitSamePadConv1d(tnn.Conv1d):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, padding=0, **kwargs)
        self.same_padding = get_padding_value(self.kernel_size[0])

    def forward(self, input):
        if self.same_padding != (0, 0):
            input = torch.nn.functional.pad(input, self.same_padding)
                ...
                if sinc_input or apply_window_to_root:
                    ...
                    if stride == 1:
                        ...
                        conv = tnn.Conv1d(
                            ...
                            stride=stride,
                            bias=conv_bias,
                            padding=np.ceil(stride / 2).astype(int),
                        )
