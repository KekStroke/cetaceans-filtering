...
def main(args):
    # Warm up resampy before worker threads start.
    # This avoids lazy import / initialization races inside librosa -> resampy.
    import numpy as _np
    _ = resampy.resample(
        _np.zeros(args.resample_rate, dtype="float32"),
        args.resample_rate,
        args.resample_rate,
        filter="kaiser_best",
    )
    print("resampy warmup OK", flush=True)
