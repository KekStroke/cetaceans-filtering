#!/usr/bin/env python3
"""SHAP-style frequency-band attribution for Anvar's animal2vec encoder (8kHz, Nyquist 4kHz).
...
Memory-safe: 1s clips, one model, GPU. Run: ~/a2v_env/bin/python a2v_shap.py <slim_ckpt>"""
import sys, time, json
sys.path.insert(0, "/mnt/c/Users/Iaroslav/CETACEANS/a2v_validation"); sys.path.insert(0, "/home/yarix/a2v")
...
SR = 8000; NB = 8; BANDS = [(i * 500, (i + 1) * 500) for i in range(NB)]   # 0..4kHz, 8 bands
ckpt = sys.argv[1] if len(sys.argv) > 1 else "/home/yarix/a2v_ckpts/ckpt25k_slim.pt"

model = E.load_model(ckpt)
items = E.gather_kclass("/mnt/c/Users/Iaroslav/CETACEANS/new_training_data", 100)
...
plt.tight_layout(); plt.savefig("/mnt/c/Users/Iaroslav/CETACEANS/a2v_validation/a2v_shap.png", dpi=120)
